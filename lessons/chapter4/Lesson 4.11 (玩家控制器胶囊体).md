...
在旧的代码中，因为没有水平面的存在，所以设计为玩家在 3D 空间中如同驾驶飞机一样，w 是往屏幕里走，s 是往屏幕外面走，这很正常。

但是在水平面上，玩家低头按住 W 时，我们不能穿过地面，你会发现完全走不动路。仰头同理。  
所以 FPS 游戏中不是这样移动，玩家的 WASD 是针对水平面，低头仰头和水平直视移动速度是一样的。我们要改进目前这套系统。

在 `PlayerControllerComponent::Update(float deltaTime)`中：
```cpp
auto& inputManager = Engine::GetInstance().GetInputManager();
auto rotation = m_owner->GetRotation();

// 初始化Y轴旋转四元数为当前旋转角度对应的四元数
...
if (inputManager.IsMousePositionChanged())
{
    ...
    float yDeltaAngle = -deltaX * m_sensitivity * deltaTime;
    m_yRot += yDeltaAngle;
    yRot = glm::angleAxis(glm::radians(m_yRot), glm::vec3(0.0f, 1.0f, 0.0f));
...
glm::vec3 front = glm::normalize(yRot * glm::vec3(0.0f, 0.0f, -1.0f));
glm::vec3 right = glm::normalize(yRot * glm::vec3(1.0f, 0.0f, 0.0f));
```
我们只使用 Y轴 方向的旋转来计算应该移动的方向。
