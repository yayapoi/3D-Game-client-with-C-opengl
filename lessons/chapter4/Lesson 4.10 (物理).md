...
我们将创建一个**PhysicsManager**来拥有和控制 bullet 世界。
...
* **设置转换矩阵**仅在添加到世界**之前有效**，因为一旦模拟，Bullet会在每一帧都覆盖它们。
...
### 6.1 文件
创建`scene/components/PhysicsComponent.h/.cpp`。它在初始化时同步变换矩阵，对于动态体，每帧都会拉回变换。
...
* `Init`：同步从`GameObject`到`RigidBody`的初始转换矩阵，然后将本体添加到物理世界：
```cpp
...
```
* `Update`：对于动态物体，从物理引擎同步转换矩阵回`GameObject`：
```cpp
...
* 在接触时，Bullet 会解决碰撞，箱子会放在地上。
* 同步变换矩阵确保游戏对象在视觉上更新以匹配物理模拟。
