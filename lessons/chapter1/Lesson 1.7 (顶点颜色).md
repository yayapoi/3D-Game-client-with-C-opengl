...
之前，我们有一个`std::vector<float>`类型的`vertices`变量，它仅存储三角形的顶点位置和颜色。我们将 **重用此相同的向量** ，但对其进行修改以表示 **具有四个顶点的矩形**：
...
OpenGL将所有内容都渲染为三角形。要渲染一个矩形，我们必须将其定义为 **两个三角形**。如果我们简单地重复共享顶点，就会造成数据重复，这非常低效——尤其是对于复杂模型而言。
...
运行程序并观察最终结果——一个由插值顶点颜色绘制的矩形，受统一颜色乘数的影响。
